...
\${ "2" * 5 } = ${ "2" * 5 }<br/>
\${ null + 1 } = ${ null + 1 }<br/>
<!-- (+)연산자 만으로는 이어붙이기 연산을 할 수 없어 사칙연산으로 이러난 값들을 출력한다. -->
</h3>
...
<!-- EL 을 이용할 때 서버객체의 내용을 출력하고자 할 때, 어떤 객체인지 명시하지 않고, 애트리뷰터 이름만 사용한다면,
사용하려는 애트리 뷰트가 있는 객체를 순서(pageContext -> request -> session -> application)대로 검사하여 가장 먼저
발견된 내용이 출력됩니다.
